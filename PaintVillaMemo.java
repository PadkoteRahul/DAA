...
public class PaintVillaMemo 
{
    ...
    public int minCost(int[][] costs) 
	{
        ...
        this.costs = costs;
        return Math.min(paintCost(0, 0, dp), Math.min(paintCost(0, 1, dp), paintCost(0, 2, dp)));
    }

    private int paintCost(int n, int color, int [][] dp) 
	{
		...
		if (color == 0) // Red
            totalCost += Math.min(paintCost(n + 1, 1, dp), paintCost(n + 1, 2, dp));
		else if (color == 1) // Green
            totalCost += Math.min(paintCost(n + 1, 0, dp), paintCost(n + 1, 2, dp));
		else // Blue
			totalCost += Math.min(paintCost(n + 1, 0, dp), paintCost(n + 1, 1, dp));
	...
	public static void main(String args[] ) {
		...
		System.out.println(new PaintVillaMemo().minCost(costs));
	}
}
